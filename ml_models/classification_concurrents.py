...
# dossier de sortie (préserve l'existant mais permet override)
OUTPUT_DIR = os.getenv(
    "OUTPUT_DIR",
    r"C:\Users\Hazem Bouchouicha\Downloads\Sougui_outputs",
)
...
# ============================================================
fact = pd.read_sql(f'SELECT * FROM {DW_SCHEMA}."FactConcurrents"', engine)
product = pd.read_sql(f'SELECT * FROM {DW_SCHEMA}."DimProduct"', engine)
category = pd.read_sql(f'SELECT * FROM {DW_SCHEMA}.dim_category', engine)
concurrent = pd.read_sql(f'SELECT * FROM {DW_SCHEMA}.dim_concurrents', engine)
...
if os.getenv("EXPORT_PT", "0") in {"1", "true", "True"}:
    artifact_path = os.getenv(
        "MODEL_ARTIFACT_PATH",
        os.path.join(OUTPUT_DIR, "classification_concurrents_model.pt"),
    )
    metrics = {
    ...
    export_info = save_pt_artifact(
        ...
        metadata={
            "model_name": "classification_concurrents",
            "framework": "sklearn",
            "measures": metrics,
        },
    )
