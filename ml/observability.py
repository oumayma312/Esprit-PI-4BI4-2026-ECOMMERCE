...
# -- Churn-specific helpers (extend the same metric namespace) --
...
def record_churn_prediction(
    ...
    **payload: Any,
    ...
    """Record a churn prediction event with structured logging."""
    log_event(
        logging.INFO,
        "churn_prediction",
        model=model,
...
def publish_churn_metrics(
    ...
    """Publish churn-specific model metrics using the shared metric namespace."""
    if avg_probability is not None:
        set_model_metric(model, "avg_churn_probability", avg_probability)
    if high_risk_rate is not None:
        set_model_metric(model, "high_risk_rate", high_risk_rate)
    if prediction_count is not None:
        set_model_metric(model, "prediction_count", prediction_count)
