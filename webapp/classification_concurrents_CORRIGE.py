...
def run_concurrent_classification(
    db_user="bi",
    db_password="bi123",
    db_host="localhost",
    db_port="5435",
...
# =============================================================================
    ...
    return {
        ...
        "roc_auc": float(roc_auc_rf if best_model_name == "Random Forest" else roc_auc_lr),
        "f1_weighted": float(f1_score(y_test, best_y_pred, average="weighted", zero_division=0)),
        "missing_categories": int(len(missing_cats)),
        "recommended_categories_count": int(len(cat_recommendation)),
        "recommended_products_count": int(len(prod_recommendation)),
        "product_recommendations": prod_recommendation.head(10).to_dict(orient="records"),
        "category_recommendations": cat_recommendation.head(10).to_dict(orient="records"),
    }
