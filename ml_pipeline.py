"""Shared ML pipeline module — churn prediction + customer segmentation.

Extracted from streamlit_app.py to be reusable by FastAPI, LangChain tools,
and the Streamlit app itself.

Usage:
    from ml_pipeline import MLPipeline

    pipeline = MLPipeline()  # loads artifacts once
...
# ---------------------------------------------------------------------------
# Constants (same as streamlit_app.py)
# ---------------------------------------------------------------------------
...
class MLPipeline:
    ...
    Loads models, scalers, and config once (cached). Provides single-row and
    ...
    def __init__(self, artifacts_dir: str = "artifacts"):
        ...
        if missing:
            raise FileNotFoundError("Missing ML artifacts:\n" + "\n".join(missing))
        ...
        self.decision_threshold = float(self._config.get("decision_threshold", 0.5))
    ...
    # ------------------------------------------------------------------
    # Internal inference (mirrors streamlit_app.py run_inference)
    # ------------------------------------------------------------------
    ...
    def run_inference(
        ...
        out["recommendation"] = out.apply(
            lambda r: self._business_recommendation(r["churn_probability"], r["segment"]),
            axis=1,
        )
    ...
    def _ensure_log_features(self, df: pd.DataFrame) -> pd.DataFrame:
        out = df.copy()
        for base_col in self._config["log_features"]:
    ...
    def _encode_channel_dummies(
        ...
        out = df.copy()
        dummy_cols = self._config["churn_channel_dummy_columns"]
    ...
    def derive_features_from_transactions(
        ...
        # Validate required columns (accept both original FK names and DB names)
        ...
        if not required_cols.issubset(set(df.columns)) and not alt_cols.issubset(set(df.columns)):
            raise ValueError("Missing transaction columns: need dateFK/transaction_date, total_ttc, quantity, price, channelFK, productFK")
        # Normalize: if transaction_date exists, rename to dateFK
        ...
        # Parse dates (strict YYYYMMDD first, then loose)
        ...
        # Monetary trend (last 3 tx mean / overall mean)
        last3_spend = (
...
# ---------------------------------------------------------------------------
# Lazy-loaded singleton (for FastAPI / Streamlit to share)
# ---------------------------------------------------------------------------

@lru_cache(maxsize=1)
def get_pipeline(artifacts_dir: str = "artifacts") -> MLPipeline:
    """Return a cached MLPipeline instance."""
    return MLPipeline(artifacts_dir)


# ---------------------------------------------------------------------------
# scikit-learn imports (needed for PCA plot)
# ---------------------------------------------------------------------------
