...
class MlDataRepository:
    ...
    def load_customer_behavior_frame(self) -> pd.DataFrame:
        fact = self._load_table(
            "sales_fact",
            ["FactVentee", "FactVente", "factventee", "factvente"],
            ["FactVentee.csv", "Fact_Vente.csv"],
        ...
        customers = self._load_table(
            "dim_customer",
            ["DimCustomer", "dim_customer", "dimcustomer"],
            ["DimCustomer.csv", "dim_customer.csv"],
        )
        ...
        dim_channel_pk = _pick_column(dim_channel.columns, ["channelpk", "canalpk", "channel_fk", "id"])
        dim_channel_value = _pick_column(dim_channel.columns, ["channel", "canal", "channel_name", "name"])
        if dim_channel_pk and dim_channel_value:
            channel_map = dim_channel[[dim_channel_pk, dim_channel_value]].rename(
                columns={dim_channel_pk: "channelfk", dim_channel_value: "channel_name"}
            )
            channel_map["channelfk"] = _coerce_numeric(channel_map["channelfk"])
        ...
        customers = customers.rename(
            columns={
                "customerpk": "customerfk",
                "customer": "customer_name",
                "customercode": "customer_code",
            }
        )
        customer_columns = ["customerfk"]
        for column in ["customer_name", "customer_code"]:
            if column not in customers.columns:
                customers[column] = ""
            customer_columns.append(column)
        customers["customerfk"] = _coerce_numeric(customers["customerfk"])
        working = working.merge(customers[customer_columns], on="customerfk", how="left")

        working = working[working["sale_date"].notna()].copy()
        working = working[working["customerfk"].notna()].copy()
        working = working[working["customerfk"] > 0].copy()
        ...
        working["discount_value"] = working["remise"].fillna(0).clip(lower=0)
        working["channel_name"] = _clean_label(
            working["channel_name"].where(
                working["channel_name"].notna(),
                working["channelfk"].map(lambda value: f"Channel {int(value)}" if pd.notna(value) else "All channels"),
            ),
            fallback="All channels",
        )
        working["customer_name"] = _clean_label(
            working["customer_name"].where(
                working["customer_name"].notna(),
                working["customerfk"].map(lambda value: f"Customer {int(value)}" if pd.notna(value) else "Unknown"),
            )
        )
        working["customer_code"] = _clean_label(working["customer_code"], fallback="-")
        working["customerfk"] = working["customerfk"].astype(int)
        working["productfk"] = working["productfk"].fillna(0).astype(int)
        working["factventepk"] = working["factventepk"].fillna(0).astype(int)
        ...
        return working[
            ...
            columns={
                "factventepk": "transaction_id",
                "customerfk": "customer_fk",
                "channelfk": "channel_fk",
    ...
    def load_sougui_catalog_frame(self) -> pd.DataFrame:
        cache_key = "sougui_catalog"
        if cache_key in self._frame_cache:
            return self._frame_cache[cache_key].copy()

        candidates = [
            self.data_dir / "sougui_products.csv",
            self.data_dir.parent / "sougui_products.csv",
        ]
        csv_path = next((path for path in candidates if path.is_file()), None)
        if csv_path is None:
            raise FileNotFoundError("Could not locate sougui_products.csv in the workspace.")

        frame = pd.read_csv(csv_path, sep=None, engine="python", encoding="utf-8-sig")
        normalized = _normalise_columns(frame)
        ...
        for column in ["image_name", "product_name", "main_category", "subcategory", "product_url"]:
            if column not in normalized.columns:
                normalized[column] = ""
            normalized[column] = normalized[column].fillna("").astype(str).str.strip()

        normalized = normalized[normalized["image_name"] != ""].copy()
        normalized.insert(0, "product_id", np.arange(1, len(normalized) + 1))

        self._frame_cache[cache_key] = normalized
        self._source_cache[cache_key] = f"csv:{csv_path.name}"
        return normalized.copy()

    def sougui_images_dir(self) -> Path:
        candidates = [
            self.data_dir.parent / "sougui_photos",
            self.data_dir / "sougui_photos",
        ]
        for path in candidates:
            if path.is_dir():
                return path
        raise FileNotFoundError("Could not locate the sougui_photos directory in the workspace.")
