...
from .utils import normalize_column_name, parse_mixed_number
...
class DataWarehouse:
    ...
    def load(self) -> WarehouseData:
        ...
        if not csv_paths:
            raise FileNotFoundError(f"Aucun fichier CSV trouve dans: {self.data_dir}")
    ...
    def _pick_table_name(
        ...
        if required:
            raise FileNotFoundError(
                f"Aucun fichier trouve pour: {preferred_names}. Dossier: {self.data_dir}"
            )
        return None
    ...
    def _normalize_products(products: pd.DataFrame) -> pd.DataFrame:
        ...
        if "product" not in out.columns:
            out["product"] = out.get("reference", "Produit inconnu")
    ...
    def _build_product_metrics(
        ...
        if not categories.empty and "categoryfk" in metrics.columns:
            ...
            metrics["category"] = metrics["category"].fillna("Non classe")
        else:
            metrics["category"] = "Non classe"
