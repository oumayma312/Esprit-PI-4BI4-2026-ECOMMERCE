...
def _default_sougui_inputs() -> dict[str, str]:
    return {
        'product': 'collier argent',
        'avg_price': '90',
        'feat_cat_popularity': '0.5',
        'feat_price_distance': '0.1',
        'feat_occurrence_score': '1.0',
        'feat_concurrent_coverage': '1.0',
    }
...
def _parse_sougui_form(form_data: Any) -> tuple[dict[str, str], dict[str, Any] | None, dict[str, str]]:
    values = _default_sougui_inputs()
    field_errors: dict[str, str] = {}
    payload: dict[str, Any] = {}
    ...
    if not product:
        field_errors['product'] = 'Le produit est obligatoire.'
    else:
    ...
    _parse_float('avg_price', 'Le prix moyen')
    _parse_float('feat_cat_popularity', 'Le score de popularite categorie')
    _parse_float('feat_price_distance', 'La distance prix')
    _parse_float('feat_occurrence_score', 'Le score d occurrence')
    _parse_float('feat_concurrent_coverage', 'La couverture concurrentielle')
...
def _render_sougui_page(
    *,
    ...
    top_products_payload: dict[str, Any] | None = None
    categories_payload: dict[str, Any] | None = None
    if api_ctx.get('api_base'):
        try:
            sougui_health, _ = _api_request('/health/sougui', base_candidates=[api_ctx.get('api_base')])
        except Exception as exc:
            errors.append(f'Impossible de lire la sante Sougui: {exc}')
        try:
            predictions_payload, _ = _api_request('/predict/sougui/products', base_candidates=[api_ctx.get('api_base')])
        except Exception as exc:
            errors.append(f'Impossible de lire les predictions Sougui: {exc}')
        try:
            top_products_payload, _ = _api_request('/top-products/sougui', base_candidates=[api_ctx.get('api_base')])
        except Exception as exc:
            errors.append(f'Impossible de lire les top produits Sougui: {exc}')
        try:
            categories_payload, _ = _api_request('/categories/sougui', base_candidates=[api_ctx.get('api_base')])
        except Exception as exc:
            errors.append(f'Impossible de lire les categories Sougui: {exc}')
    ...
    top_products = []
    if isinstance(top_products_payload, dict):
        top_products = top_products_payload.get('data') or []

    categories = []
    if isinstance(categories_payload, dict):
        categories = categories_payload.get('data') or []
    ...
    return render_template(
        ...
        health=api_ctx.get('health'),
        models=api_ctx.get('models'),
        sougui_health=sougui_health,
        result=result,
        predictions=predictions,
        top_products=top_products,
        categories=categories,
        input_values=input_values or _default_sougui_inputs(),
        field_errors=field_errors or {},
        errors=errors,
    )
...
@app.get('/sougui')
def sougui() -> str:
    return _render_sougui_page()


@app.post('/sougui/predict')
def sougui_predict() -> str:
    input_values, payload, field_errors = _parse_sougui_form(request.form)
    if payload is None:
        return _render_sougui_page(
            input_values=input_values,
            field_errors=field_errors,
            local_errors=['Veuillez corriger les champs du formulaire Sougui.'],
        )

    error = None
    result = None
    try:
        result, _ = _api_request('/predict/sougui', method='POST', payload=payload)
    except Exception as exc:
        error = f'La prediction Sougui a echoue: {exc}'
