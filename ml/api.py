...
from ml.models.schemas import (
    ...
    CustomerChurnScenarioRequest,
    PromoteWindowRequest,
    ProductMatcherCompareRequest,
    SellWindowRequest,
...
def build_ml_router(config: AppConfig) -> APIRouter:
    ...
    @router.get("/customers/churn/summary")
    def get_customer_churn_summary() -> dict[str, object]:
        try:
            return hub.churn.summary()
        except (MlServiceError, FileNotFoundError) as exc:
            raise translate_error(exc) from exc

    @router.post("/customers/churn/predict")
    def predict_customer_churn(payload: CustomerChurnScenarioRequest) -> dict[str, object]:
        try:
            return hub.churn.predict(payload.model_dump(by_alias=True))
        except MlServiceError as exc:
            raise translate_error(exc) from exc

    @router.get("/matcher/overview")
    def get_matcher_overview() -> dict[str, object]:
        try:
            return hub.matcher.overview()
        except (MlServiceError, FileNotFoundError) as exc:
            raise translate_error(exc) from exc

    @router.get("/matcher/catalog")
    def search_matcher_catalog(
        query: str = Query(default="", max_length=255),
        category: str = Query(default="", max_length=120),
        limit: int = Query(default=24, ge=1, le=60),
    ) -> dict[str, object]:
        try:
            return hub.matcher.search(query=query, category=category, limit=limit)
        except MlServiceError as exc:
            raise translate_error(exc) from exc

    @router.post("/matcher/compare")
    def compare_matcher_products(payload: ProductMatcherCompareRequest) -> dict[str, object]:
        try:
            return hub.matcher.compare(
